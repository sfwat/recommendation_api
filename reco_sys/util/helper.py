...
def getCustSeniority(user):
    min_value = 0.
    max_value = 256.
    range_value = max_value - min_value
    missing_value = 0.
    cust_seniority = user.get('antiguedad',"")
...
def processData(user):
    ...
    x_vars = []
    for col in cat_cols:
        x_vars.append(getIndex(user, col))
    x_vars.append(getAge(user))
    x_vars.append(getCustSeniority(user))
    x_vars.append(getRent(user))
...
def predict(user):
    ...
    x_vars_list, y_vars_list, cust_dict = processData(user)
...
def replace_user_date(user):
    temp_user_object = user_object
    user_data_translation = {
                "gender": "sexo",
                "seniority": "antiguedad",
                "segment": "segmento",
                "relationship_type": "tiprel_1mes",
                "activity_level": "ind_actividad_cliente",
                "annual_income": "renta",
                "region": "nomprov"}

    for key in user_data_translation:
        if key in user:
...
def json_validation(user):
    ...
    if jsonschema.validate(user, schema):
        return True
    return False